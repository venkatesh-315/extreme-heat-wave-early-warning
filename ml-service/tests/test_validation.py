...
Validates that invalid inputs return HTTP 400 Bad Request with structured details and no stack traces.
"""
...
class TestValidationGuardrails(unittest.TestCase):
    ...
    def test_reject_missing_required_fields(self):
        """Verify HTTP 400 error when required fields are missing."""
        # Missing humidity
        response = self.client.post("/ml/predict", json={"temperature": 42.0})
        self.assertEqual(response.status_code, 400)
        data = response.json()
        self.assertEqual(data["error"], "InvalidInput")
    ...
    def test_reject_negative_humidity(self):
        """Verify negative humidity is rejected with HTTP 400."""
    ...
    def test_reject_humidity_over_100(self):
        """Verify humidity > 100% is rejected with HTTP 400."""
        payload = {"temperature": 40.0, "humidity": 105.0}
        response = self.client.post("/ml/predict", json=payload)
        self.assertEqual(response.status_code, 400)

    def test_reject_negative_wind_and_solar(self):
        """Verify negative wind speed and solar radiation are rejected with HTTP 400."""
